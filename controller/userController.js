...
exports.userInvitation = async (req, res) => {
    ...
    try {
        ...
        if (existingUser) {
            return res.status(409).json({ error: "Email already exists with someone else." });
        }
        ...
        // Send response
        res.json({ message: "Password reset mail sent successfully", user: userdata });
...
exports.TenantSignup = async (req, res) => {
    try {
        ...
        console.log('Tenant data:', existingTenant);  // Logging existing tenant data
...
exports.tenantsforgotPass = async (req, res) => {
